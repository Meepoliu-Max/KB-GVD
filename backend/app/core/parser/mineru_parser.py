...
class MineruParser(BaseParser):
    ...
    def _extract_tables(markdown: str) -> list[Table]:
        """提取 HTML 表格块及其位置。"""
        from app.core.parser.base import Table
...
class ParserFactory:
    ...
    目前只有 MineruParser，未来可扩展其他解析器。
    """

    _parser: MineruParser | None = None

    @classmethod
    def get_parser(cls, file_type: FileType | None = None) -> BaseParser:
        """获取解析器实例。

        Args:
            file_type: 文件类型（当前所有类型都用 MineruParser，可忽略）
        """
