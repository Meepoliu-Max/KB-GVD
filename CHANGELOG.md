...
- **管理后台**（`/admin/*`，需 super\_admin / admin 角色）：
...
- **任务归属与 Token 统计**：上传自动关联 user\_id；任务完成后回写 LLM Token 消耗（按任务持久化）；任务列表按用户隔离（普通用户仅见自己的任务）。
...
- 首个正式版本：RAG 四阶流水线（Clean → Chunk → QA → Tag）、Web UI、CLI、SDK、
  导出格式（coze\_qa / coze\_text / dify\_qa / dify\_text / dify\_jsonl / json）、断点续跑。
