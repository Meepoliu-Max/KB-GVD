...
基于 OpenAI 兼容 SDK 调用 DeepSeek API，提供：
- 模型切换：deepseek-v4-flash（默认，快速）/ deepseek-v4-pro（复杂任务）
...
模型说明（2026 年官方文档）：
- deepseek-v4-flash：非思考模式默认，速度快，1M 上下文，并发 2500
  - 旧名 deepseek-chat（2026/07/24 弃用）
- deepseek-v4-pro：复杂推理，3 元/百万输入，6 元/百万输出，并发 500
  - 旧名 deepseek-reasoner（2026/07/24 弃用）

JSON Output 已知问题（官方文档）：
  使用 response_format=json_object 时，API 有概率返回空 content。
  本客户端用 EmptyContentError 捕获并自动重试。

用法：
    from app.core.llm import DeepSeekClient

    client = DeepSeekClient()  # 从环境变量读取 API Key
...
# DeepSeek 官方 BASE URL（OpenAI 兼容格式）
...
class LLMConfig:
    ...
    api_key: str = ""  # 空则从 DEEPSEEK_API_KEY 环境变量读取
...
def _get_config(api_key: str | None, config: LLMConfig | None) -> LLMConfig:
    ...
    if not cfg.api_key:
        raise ConfigurationError(
            "未配置 API Key。请传入 api_key 参数或设置 DEEPSEEK_API_KEY 环境变量。"
        )
    return cfg
...
class DeepSeekClient:
    """DeepSeek LLM 客户端（同步 + 异步）。

    封装 OpenAI 兼容 SDK，提供 JSON Output / 模型切换 / 重试 / 超时。
